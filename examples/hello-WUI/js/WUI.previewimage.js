(function($, window) {
	...
	var template = '<div id="{{id}}" class="WUI-slider WUI-preview-image WUI-fullscreen"><div class="WUI-preview-header">{{header}}</div><div class="WUI-slider-group"></div><div class="WUI-preview-footer WUI-hidden">{{footer}}</div><div class="WUI-preview-loading"><span class="WUI-spinner WUI-spinner-white"></span></div></div>';
	var itemTemplate = '<div class="WUI-slider-item WUI-zoom-wrapper {{className}}"><div class="WUI-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="WUI-zoom"></div></div>';
	var defaultGroupName = '__DEFAULT';
	var div = document.createElement('div');
	var imgId = 0;
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			id: '__WUI_PREVIEWIMAGE',
			zoom: true,
			header: '<span class="WUI-preview-indicator"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			if (self.options.zoom) {
				var lastZoomerEl = self.element.querySelector('.WUI-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
				if (lastZoomerEl) {
	...
	proto.addImage = function(img) {
		...
		if (img.__WUI_img_data && img.__WUI_img_data.src === src) { //已缓存且图片未变化
			this.groups[group].push(img.__WUI_img_data);
		} else {
	...
	proto.open = function(index, group) {
		...
		if (typeof index === "number") {
			...
			group = group || defaultGroupName;
			this.addImages(group, index); //刷新当前group
			this.openByGroup(this.groups[group].indexOf(index.__WUI_img_data), group);
		}
	};
