...
(function($, document) {
	...
	var ImageViewer = $.ImageViewer = $.Class.extend({
		...
		createViewer: function() {
			var self = this;
			self.viewer = $.dom("<div class='WUI-imageviewer'><div class='WUI-imageviewer-mask'></div><div class='WUI-imageviewer-header'><i class='WUI-icon WUI-icon-closeempty WUI-imageviewer-close'></i><span class='WUI-imageviewer-state'></span></div><i class='WUI-icon WUI-icon-arrowleft  WUI-imageviewer-left'></i><i class='WUI-icon WUI-icon-arrowright WUI-imageviewer-right'></i></div>");
			self.viewer = self.viewer[0] || self.viewer;
			//self.viewer.style.height = screen.height;
			self.closeButton = self.viewer.querySelector('.WUI-imageviewer-close');
			self.state = self.viewer.querySelector('.WUI-imageviewer-state');
			self.leftButton = self.viewer.querySelector('.WUI-imageviewer-left');
			self.rightButton = self.viewer.querySelector('.WUI-imageviewer-right');
			self.mask = self.viewer.querySelector('.WUI-imageviewer-mask');
		...
		next: function() {
			...
			setTimeout(function() {
				self.currentItem.classList.remove('WUI-imageviewer-item-center');
				self.currentItem.classList.add('WUI-imageviewer-item-left');
				newItem.classList.remove('WUI-imageviewer-item-right');
				newItem.classList.add('WUI-imageviewer-item-center');
		...
		prev: function() {
			...
			setTimeout(function() {
				self.currentItem.classList.remove('WUI-imageviewer-item-center');
				self.currentItem.classList.add('WUI-imageviewer-item-right');
				newItem.classList.remove('WUI-imageviewer-item-left');
				newItem.classList.add('WUI-imageviewer-item-center');
				self.oldItem = self.currentItem;
				self.currentItem = newItem;
				// TODO: 临时,稍候将调整
		...
		disposeImage: function(all) {
			var sel = '.WUI-imageviewer-item-left,.WUI-imageviewer-item-right';
			if (all) sel += ",.WUI-imageviewer-item";
		...
		createImage: function(index, type) {
			...
			self.index = index;
			var item = $.dom("<div class='WUI-imageviewer-item'></div>")[0];
			item.appendChild($.dom('<span><img src="' + self.images[self.index].src + '"/></span>')[0]);
			item.classList.add('WUI-imageviewer-item-' + type);
